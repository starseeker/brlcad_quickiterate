...
struct ged_bv_data {
    ...
    struct ged *gedp; /**< @brief back-pointer to owning GED session; used by the per-solid s_free_callback to clear the illumination NodeSensor when the freed solid is the one being illuminated, without requiring gedp to be passed through the BSG freeing path (Phase 7 Step 9). */
    void *u_data;
};
