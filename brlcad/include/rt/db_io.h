...
/**
 * This routine is called by the RT_GET_SEG macro when the freelist is
 * exhausted.  Rather than simply getting one additional structure, we
 * get a whole batch, saving overhead.  malloc() locking is done in
 * bu_malloc.
 *
 * Deprecated: the declaration is now in rt/seg.h.  This stub is
 * retained for source compatibility until Phase 8 removes the
 * directory from public view.
 */
/* RT_EXPORT extern void rt_alloc_seg_block(struct rt_seg_pool *pool); -- see rt/seg.h */
