 ...
 * replaces the legacy pattern of returning a raw @c bu_ptbl of @c bsg_node
 * pointers from pick/selection functions.
 *
 * Pick records carry:
 *  - the matched scene node;
 ...
 *
 * Fields:
 *   pr_node         — matched BSG node (borrowed; not owned by the record).
 ...
 * existing code that expects a node table.
 *
 * The returned table is populated with pointers borrowed from @p res; the
 * nodes are NOT copied.  The caller is responsible for @c bu_ptbl_free()
 * (but NOT for freeing the nodes themselves).
 *
 * @p out must be initialised by the caller (BU_PTBL_INIT_ZERO or
 * bu_ptbl_init()).
 *
 * No-op if @p res or @p out is NULL.
 */
BSG_EXPORT extern void
