 ...
 *
 * @brief
 * Generic BSG node accessors over the current scene-node backing storage.
 *
 * BSG is the preferred public scene API; legacy scene-object storage remains a
 * transitional backing implementation while the dedicated node storage model is
 * being introduced.
 */
/** @{ */
/* @file bsg/node.h */
...
/**
 * Return the bounding-sphere radius for @p n (@c s_size).
 *
...
/**
 * Set the bounding-sphere radius for @p n (@c s_size).
 *
 ...
 *
 * Maps to the legacy @c s_displayobj flag.  Objects with this flag
 ...
 *
 * This is the typed BSG wrapper for the legacy @c s_update_callback
 * signature.
 */
 ...
 *
 * Maps to @c s_free_callback.  Pass NULL to clear any existing
 ...
 *
 * Maps to @c s_update_callback.  Pass NULL to clear any existing
 * callback.  No-op if @p n is NULL.
 */
BSG_EXPORT extern void
...
/**
 * Set the legacy E-flag on @p n (@c s_old.s_Eflag).
 *
