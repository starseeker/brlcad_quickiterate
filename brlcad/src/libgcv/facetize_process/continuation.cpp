...
continuation_mesh(struct rt_bot_internal **obot, struct db_i *dbip, const char *objname, tess_opts *s, point_t seed)
{
    ...
    double feature_size = (s->runtime.cm.sample.feature_size > 0) ? s->runtime.cm.sample.feature_size : 2*s->runtime.cm.sample.avg_thickness;
    ...
    params.max_time = s->runtime.cm.max_time;
    params.max_cycle_time = s->runtime.cm.max_cycle_time;
    params.verbosity = 1;
    params.minimum_free_mem = FACETIZE_MEMORY_THRESHOLD;

    while (!pret && (feature_size > 0.9*s->runtime.cm.sample.target_feature_size) && fatal_error_cnt < 8 ) {
	...
	if (pret || candidate->num_faces < successful_bot_count || delta < 2) {
	    ...
	    if (pret != 2 && !(s->runtime.cm.sample.feature_size > 0)) {
	    ...
	    if (bot && bot->faces) {
		if (s->runtime.cm.sample.feature_size <= 0) {
		    bu_log("CM: unable to polygonize at target size (%g), using last successful BoT with %d faces, feature size %g\n",  s->runtime.cm.sample.target_feature_size, (int)bot->num_faces, successful_feature_size);
		} else {
	...
	total_elapsed += delta;
	if (s->runtime.cm.max_cycle_time < delta*2) {
	    if (total_elapsed + s->runtime.cm.max_cycle_time > s->runtime.cm.max_time) {
		feature_size = successful_feature_size;
		break;
	    }
	} else {
	    if (total_elapsed + delta*2 > s->runtime.cm.max_time) {
		feature_size = successful_feature_size;
		break;
	    }
	}
    ...
    /* do decimation */
    {
	double d_feature_size = (s->runtime.cm.sample.d_feature_size > 0) ? s->runtime.cm.sample.d_feature_size : 1.5 * feature_size;
	*obot = bot;
    ...
    if (fabs(s->runtime.cm.sample.pnts_bbox_vol - bot_bbox_vol) > s->runtime.cm.sample.pnts_bbox_vol * 0.5) {
