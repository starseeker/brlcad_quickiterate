 ...
 * TRI_CONSTRAINED_DELAUNAY.  On success (return 0) fu has been killed and
 * replaced by triangle faceuses connected back to the surrounding shell via
 * nmg_je().  On failure (return 1) fu is completely unchanged and the caller
 * should fall back to the ear-clip path.
 */
static int
nmg_tri_fu_bg(struct faceuse *fu, struct bu_list *UNUSED(vlfree), const struct bn_tol *tol)
{
    struct loopuse *lu;
    struct edgeuse *eu;
    ...
    for (BU_LIST_FOR(lu, loopuse, &fu->lu_hd)) {
	if (BU_LIST_FIRST_MAGIC(&lu->down_hd) != NMG_EDGEUSE_MAGIC) continue;
	if (lu->orientation == OT_SAME) {
	    int vc = 0;
	    for (BU_LIST_FOR(eu, edgeuse, &lu->down_hd)) vc++;
	    if (vc > n_outer) { n_outer = vc; outer_lu = lu; }
    ...
    if (!outer_lu || n_outer < 3) return 1;
    if (n_outer == 3 && nholes == 0) return 1;

    /* ---- projection axes: u_ax x v_ax = fu_normal ---- */
    vect_t fu_normal;
    NMG_GET_FU_NORMAL(fu_normal, fu);
    vect_t u_ax, v_ax;
    bn_vec_ortho(u_ax, fu_normal);
    VCROSS(v_ax, fu_normal, u_ax);  /* u_ax x v_ax = fu_normal => CCW = outward */

    /* project origin from first outer vertex */
    eu = BU_LIST_FIRST(edgeuse, &outer_lu->down_hd);
    point_t base_pt;
    VMOVE(base_pt, eu->vu_p->v_p->vg_p->coord);

    /* ---- build combined 2-D point array ---- */
    int npts = n_outer + n_inner_total;
    point2d_t *pts = (point2d_t *)bu_calloc(npts, sizeof(point2d_t), "nmg_tri_fu_bg pts");
    struct vertex **idx_to_vert = (struct vertex **)bu_calloc(npts, sizeof(struct vertex *), "nmg_tri_fu_bg verts");

    int *poly = (int *)bu_calloc(n_outer, sizeof(int), "nmg_tri_fu_bg poly");
    int idx = 0;
    ...
    int **holes_arr  = nholes > 0 ? (int **)bu_calloc(nholes, sizeof(int *), "nmg_tri_fu_bg holes_arr") : NULL;
    size_t *holes_npts = nholes > 0 ? (size_t *)bu_calloc(nholes, sizeof(size_t), "nmg_tri_fu_bg holes_npts") : NULL;
    int h = 0;
    for (BU_LIST_FOR(lu, loopuse, &fu->lu_hd)) {
	if (BU_LIST_FIRST_MAGIC(&lu->down_hd) != NMG_EDGEUSE_MAGIC) continue;
	...
	holes_arr[h] = (int *)bu_calloc(n_hole, sizeof(int), "nmg_tri_fu_bg hole");
	for (int j = 0; j < n_hole; j++) holes_arr[h][j] = hole_start + j;
	holes_npts[h] = (size_t)n_hole;
	h++;
    }
	...
	poly, (size_t)n_outer,
	(const int **)holes_arr, holes_npts, (size_t)nholes,
	NULL, 0,
	pts, (size_t)npts,
	TRI_CONSTRAINED_DELAUNAY);

    if (bg_ret != 0 || num_tri <= 0 || !tri_faces) goto fail;

    {
	/* ---- build new triangle faces, then kill old face ---- */
	struct shell *s = nmg_find_s_of_fu(fu);

	/* Kill original face BEFORE creating new ones so vertex vu_hd lists
	 * only contain side-wall vertexuses when we search for mates below. */
	(void)nmg_kfu(fu);  /* fu is now a dangling pointer – do not use */

	struct faceuse **new_fus = (struct faceuse **)bu_calloc(num_tri, sizeof(struct faceuse *), "nmg_tri_fu_bg new_fus");
	int n_new = 0;
	...
	for (int k = 0; k < num_tri; k++) {
	    ...
	    if (i0 < 0 || i0 >= npts || i1 < 0 || i1 >= npts || i2 < 0 || i2 >= npts) continue;
	    ...
	    if (!tv[0] || !tv[1] || !tv[2]) continue;
	    if (tv[0] == tv[1] || tv[1] == tv[2] || tv[0] == tv[2]) continue;
	...
	/* ---- glue new triangles to surrounding side-wall faces ---- */
	for (int i = 0; i < n_new; i++) {
	    struct faceuse *nfu = new_fus[i];
	    struct loopuse *nlu;
	    for (BU_LIST_FOR(nlu, loopuse, &nfu->lu_hd)) {
		if (BU_LIST_FIRST_MAGIC(&nlu->down_hd) != NMG_EDGEUSE_MAGIC) continue;
		if (nlu->orientation != OT_SAME) continue;
		struct edgeuse *neu;
		for (BU_LIST_FOR(neu, edgeuse, &nlu->down_hd)) {
		    /* skip already-glued edges */
		    if (neu->radial_p != neu->eumate_p) continue;

		    struct vertex *v1 = neu->vu_p->v_p;
		    struct vertex *v2 = neu->eumate_p->vu_p->v_p;

		    /* side-wall edge goes v2→v1 (opposite direction) */
		    struct vertexuse *vu;
		    for (BU_LIST_FOR(vu, vertexuse, &v2->vu_hd)) {
			if (*vu->up.magic_p != NMG_EDGEUSE_MAGIC) continue;
			struct edgeuse *cand = vu->up.eu_p;
			if (cand->e_p == neu->e_p) continue;          /* same edge */
			if (cand->eumate_p->vu_p->v_p != v1) continue; /* wrong dest */
			if (cand->radial_p != cand->eumate_p) continue; /* already glued */
			/* cand goes v2→v1, neu goes v1→v2: correct mate pair */
			nmg_je(neu, cand);
			break;
		    }
		}
	...
	bu_free(new_fus, "nmg_tri_fu_bg new_fus");
    }

    bu_free(tri_faces, "nmg_tri_fu_bg tri_faces");
    bu_free(idx_to_vert, "nmg_tri_fu_bg idx_to_vert");
    bu_free(pts, "nmg_tri_fu_bg pts");
    bu_free(poly, "nmg_tri_fu_bg poly");
    for (int i = 0; i < nholes; i++) bu_free(holes_arr[i], "nmg_tri_fu_bg hole");
    if (holes_arr)  bu_free(holes_arr,   "nmg_tri_fu_bg holes_arr");
    if (holes_npts) bu_free(holes_npts,  "nmg_tri_fu_bg holes_npts");
    return 0;  /* success */

fail:
    if (tri_faces)  bu_free(tri_faces,   "nmg_tri_fu_bg tri_faces");
    bu_free(idx_to_vert, "nmg_tri_fu_bg idx_to_vert");
    bu_free(pts,    "nmg_tri_fu_bg pts");
    bu_free(poly,   "nmg_tri_fu_bg poly");
    for (int i = 0; i < nholes; i++) if (holes_arr[i]) bu_free(holes_arr[i], "nmg_tri_fu_bg hole");
    if (holes_arr)  bu_free(holes_arr,   "nmg_tri_fu_bg holes_arr");
    if (holes_npts) bu_free(holes_npts,  "nmg_tri_fu_bg holes_npts");
    return 1;  /* failure – fu unchanged, use ear-clip fallback */
}
