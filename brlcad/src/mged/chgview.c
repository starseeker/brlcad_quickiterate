	...
	int did_sca)
{
    ...
    /* Keep the view pointer current for view-relative coordinate transforms */
    re->vp = view_state->vs_gvp;
...
cmd_mrot(ClientData clientData, Tcl_Interp *interp, int argc, const char *argv[])
{
	...
	{
	    int mat_edit = (s->global_editing_state == ST_O_EDIT);
	    MEDIT(s)->vp = view_state->vs_gvp;
	    rt_knob_edit_rot(MEDIT(s), view_state->vs_gvp->gv_coord,
		    view_state->vs_gvp->gv_rotate_about, mat_edit, rmat);
	    if (!mat_edit) rt_edit_process(MEDIT(s));
	    new_edit_mats(s);
	    s->update_views = 1;
	    dm_set_dirty(DMP, 1);
	}
...
cmd_rot(ClientData clientData, Tcl_Interp *interp, int argc, const char *argv[])
{
...

	{
	    int mat_edit = (s->global_editing_state == ST_O_EDIT);
	    MEDIT(s)->vp = view_state->vs_gvp;
	    rt_knob_edit_rot(MEDIT(s), coord,
		    view_state->vs_gvp->gv_rotate_about, mat_edit, rmat);
	    if (!mat_edit) rt_edit_process(MEDIT(s));
	    new_edit_mats(s);
	    s->update_views = 1;
...
cmd_tra(ClientData clientData, Tcl_Interp *interp, int argc, const char *argv[])
{
...

	{
	    int mat_edit = (s->global_editing_state == ST_O_EDIT);
	    MEDIT(s)->vp = view_state->vs_gvp;
	    rt_knob_edit_tran(MEDIT(s), coord, mat_edit, tvec);
	    if (!mat_edit) rt_edit_process(MEDIT(s));
	    new_edit_mats(s);
	    s->update_views = 1;
	    dm_set_dirty(DMP, 1);
