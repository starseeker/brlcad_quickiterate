...
dotitles(struct mged_state *s, struct bu_vls *overlay_vls)
{
    ...
    /* Step 7.6: use mged_curr_pane->mp_*_name instead of mged_curr_dm->dm_*_name */
    Tcl_SetVar(s->interp, bu_vls_addr(&s->mged_curr_pane->mp_center_name),
    ...
    Tcl_SetVar(s->interp, bu_vls_addr(&s->mged_curr_pane->mp_size_name),
    ...
    Tcl_SetVar(s->interp, bu_vls_addr(&s->mged_curr_pane->mp_aet_name),
	       bu_vls_addr(&vls), TCL_GLOBAL_ONLY);
    ...
    Tcl_SetVar(s->interp, bu_vls_addr(&s->mged_curr_pane->mp_ang_name),
	       bu_vls_addr(&vls), TCL_GLOBAL_ONLY);
	...
	Tcl_SetVar(s->interp, bu_vls_addr(&s->mged_curr_pane->mp_adc_name),
		   bu_vls_addr(&vls), TCL_GLOBAL_ONLY);
	ss_line_not_drawn = 0;
    } else {
	Tcl_SetVar(s->interp, bu_vls_addr(&s->mged_curr_pane->mp_adc_name), "", TCL_GLOBAL_ONLY);
    }
    ...
    Tcl_SetVar(s->interp, bu_vls_addr(&s->mged_curr_pane->mp_fps_name),
	       bu_vls_addr(&vls), TCL_GLOBAL_ONLY);
