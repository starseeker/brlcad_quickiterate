...
#define GET_BV_SCENE_OBJ(p, fp) { \
          if (BU_LIST_IS_EMPTY(fp)) { \
              ...
              bsg_node_u3_set((bsg_node *)(p), (void *)bdata); \
          } else { \
              p = BU_LIST_NEXT(bv_scene_obj, fp); \
              BU_LIST_DEQUEUE(&((p)->bsg.l)); \
              if (bsg_node_u3_get((bsg_node *)(p))) { \
                  struct ged_bv_data *bdata = (struct ged_bv_data *)bsg_node_u3_get((bsg_node *)(p)); \
    ...
    struct bv_scene_obj *sp)
{
    ...
    if (!sp || !bsg_node_u3_get((bsg_node *)sp))
	return;
    struct ged_bv_data *bdata = (struct ged_bv_data *)bsg_node_u3_get((bsg_node *)sp);
