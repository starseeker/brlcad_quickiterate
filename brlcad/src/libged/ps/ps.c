...
ged_ps_core(struct ged *gedp, int argc, const char *argv[])
{
    ...
    { struct bsg_camera _cm; bsg_view_get_camera(gedp->ged_gvp, &_cm);
      dl_ps(gedp->ged_gvp, fp, border, bu_vls_addr(&font), bu_vls_addr(&title), bu_vls_addr(&creator), linewidth, scale, xoffset, yoffset, _cm.model2view, _cm.perspective, _cm.eye_pos, border_red, border_green, border_blue); }
