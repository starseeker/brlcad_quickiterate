...
ged_xpush_core(struct ged *gedp, int argc, const char *argv[])
{
    ...
    db_functree(gedp->dbip, old_dp, increment_uses, increment_uses, NULL);
	...
	db_functree(gedp->dbip, dp, increment_nrefs, increment_nrefs, NULL);
    }
    ...
    /* Make new names */
    db_functree(gedp->dbip, old_dp, Make_new_name, Make_new_name, (void *)gedp);
