...
bot_adaptive_plot(bsg_shape *s, bsg_view *v)
{
    ...
    // generated by the background GeomLoader thread.  If so, clear the
    // placeholder and fall through to build a proper LoD view object.
    if (vo && !vo->draw_data) {
	...
	if (key) {
	    ...
	    // LoD still being generated - keep the placeholder.
	    return;
	}
    }
	...
	unsigned long long key = bsg_mesh_lod_key_get(d->mesh_c, dp->d_namep);
	if (!key) {
	    // The LoD cache is not yet available.  If background generation
	    // is in progress (GeomLoader), the key will arrive later and
	    // stale_mesh_shapes_for_dp() will clear this view object so the
	    // next redraw regenerates with proper LoD.  In the meantime,
	    // draw a temporary bounding-box wireframe so the user sees
	    // something immediately rather than nothing at all.
	    // If there is no background loader (e.g. ged_lod is NULL),
	    // fall back to the synchronous path below.
	    if (s->have_bbox) {
