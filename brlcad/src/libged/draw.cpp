...
{
    return (struct db_full_path *)bsg_node_uptr_get((const bsg_node *)s, 1);
}
...
scene_directory(const struct bv_scene_obj *s)
{
    ...
    return (fp) ? DB_FULL_PATH_CUR_DIR(fp) : (struct directory *)bsg_node_uptr_get((const bsg_node *)s, 0);
}
...
draw_gather_paths(struct db_full_path *path, mat_t *curr_mat, void *client_data)
{
	...
	bsg_node_uptr_set((bsg_node *)s, 1, (void *)sfp);
	bsg_node_uptr_set((bsg_node *)s, 0, DB_FULL_PATH_CUR_DIR(path));
	...
	ud->fp = (struct db_full_path *)bsg_node_uptr_get((const bsg_node *)s, 1);
	ud->dbip = dd->dbip;
	ud->tol = dd->tol;
	ud->ttol = dd->ttol;
	ud->mesh_c = dd->mesh_c;
