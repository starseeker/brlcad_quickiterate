...
who_solids_legacy_cb(struct bsg_node *sp, void *userdata)
{
    ...
    if (!ctx || !ctx->dbip || !ctx->vls || !sp)
	return 1;

    who_solids_print_scene_obj(sp, ctx->dbip, ctx->lvl, ctx->vls);
    return 1;
}
...
who_solids_impl(struct ged *gedp, int argc, const char *argv[], int subcmd_usage)
{
	...
	struct who_solids_legacy_data ctx = {gedp->dbip, lvl, gedp->ged_result_str};
	bsg_view_obj_foreach_solid(gedp, who_solids_legacy_cb, &ctx);
	bu_vls_free(&cvls);
	return BRLCAD_OK;
    }
