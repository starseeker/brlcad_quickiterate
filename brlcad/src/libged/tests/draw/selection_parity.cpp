 ...
 * information.
 */
/** @file selection_parity.cpp
 *
 * Phase D3 exit-criteria regression: verify that bsg_selection-based
 * selection is consistent with bsg_pick_result after a GED draw+pick cycle.
 *
 * Strategy:
 *   1. Open moss.g, draw the scene.
 *   2. Run a screen-center point pick; collect bsg_pick_result.
 *   3. Feed the first pick record into bsg_selection_add.
 *   4. Verify bsg_selection_count > 0 and bsg_selection_contains returns true
 *      for the picked node.
 *   5. bsg_selection_clear; verify count drops to 0.
 *
 * No display hardware required (headless GED).
 *
 * Usage: ged_test_selection_parity <directory-containing-moss.g>
 */
...
main(int ac, char *av[])
{
    ...
    bu_dir(lcache, MAXPATHLEN, BU_DIR_CURR,
	   "ged_selection_parity_cache", NULL);
    bu_mkdir(lcache);
    ...
    bu_vls_sprintf(&wname, "%s/moss_selection_parity_tmp.g",
     ...
     * ------------------------------------------------------------------ */
    struct bsg_selection *sel = v->gv_s ? v->gv_s->gv_selected : NULL;
    ASSERT(sel != NULL);
    if (!sel) {
	...
	struct bsg_pick_record *rec = bsg_pick_result_get(pr, 0);
	if (rec && rec->pr_node) {
	    bsg_selection_add(sel, rec->pr_node);
	    ASSERT(bsg_selection_count(sel) == 1);
	    ASSERT(bsg_selection_contains(sel, rec->pr_node));
	}
    }
    ...
    bu_log("D3 selection_parity: %d checks, %d failures\n", nchecks, nfails);
    return nfails ? 1 : 0;
}
