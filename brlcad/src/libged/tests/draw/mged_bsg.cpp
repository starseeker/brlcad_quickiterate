...
test_wireframe(const char *datadir)
{
    ...
    if (!v->bsg_root) {
	bu_log("FAIL: bsg_root is NULL after ged_open\n");
	ged_close(gedp);
	bu_file_delete("mged_bsg_t1.g");
	return 1;
    }
    bu_log("PASS: bsg_root created at ged_open\n");
...
/* ========================================================================== */
/* Test 2: Illumination (s_iflag == UP → object renders white)                */
/* ========================================================================== */
static int
test_illumination(const char *datadir)
{
    bu_log("\n--- Test 2: illumination (s_iflag UP → white pixels) ---\n");
	...
	bu_log("FAIL: illuminated image has %ld white pixels, baseline has %ld "
	       "(expected more white pixels when s_iflag == UP)\n",
...
test_edit_matrix(const char *datadir)
{
    ...
    mat_t edit_mat;
    MAT_COPY(edit_mat, v->gv_model2view);
    /* Apply a 0.5 unit shift in view X (pushes object to the right side) */
    edit_mat[3]  += 1.5;

    v->gv_edit_mat = edit_mat;
    do_refresh(gedp);
    capture(gedp, "mged_bsg_t3_shifted.png");

    /* Verify gv_edit_mat is cleared by... it should be cleared by caller in
     * real MGED (dozoom), but here we're calling dm_draw_objs directly which
     * doesn't clear it.  We clear it manually. */
    v->gv_edit_mat = NULL;

    int fail = 0;
    if (images_identical("mged_bsg_t3_normal.png", "mged_bsg_t3_shifted.png", 5)) {
	bu_log("FAIL: edit_mat translation produced no change in rendered output\n");
	fail = 1;
    } else {
...
test_faceplate(const char *datadir)
{
    ...
    VSET(v->gv_s->gv_center_dot.gos_line_color, 255, 255, 255);
    do_refresh(gedp);
