 ...
 * information.
 */
/** @file measure_parity.cpp
 *
 * Phase D3 exit-criteria regression: verify that bsg_measure_candidates
 * returns a distance consistent with DIST_PNT_PNT for known point pairs,
 * and that QgMeasureFilter's length1/length2 match after mr12/mr23 are
 * populated.
 *
 * Strategy:
...
main(int UNUSED(ac), char *av[])
{
    ...
    bu_log("D3 measure_parity: %d checks, %d failures\n", nchecks, nfails);
    return nfails ? 1 : 0;
}
