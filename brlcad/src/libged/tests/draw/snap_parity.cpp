 ...
 * information.
 */
/** @file snap_parity.cpp
 *
 * Phase D3 exit-criteria regression: verify that bsg_snap_point_2d and
 * bsg_snap_candidates produce consistent results after the libtclcad/libqtcad
 * snap-consumer migration.
 *
 * Strategy:
...
main(int UNUSED(ac), char *av[])
{
	...
	if (cnt > 0 && sr.sr_cnt > 0) {
	    ...
	    ASSERT(dist <= v.gv_s->gv_grid.res_h * 1.5);
	}
    ...
    bu_log("D3 snap_parity: %d checks, %d failures\n", nchecks, nfails);
    return nfails ? 1 : 0;
}
