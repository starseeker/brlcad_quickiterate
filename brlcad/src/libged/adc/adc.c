...
void ged_calc_adc_pos(struct bsg_view *gvp);
void ged_calc_adc_a1(struct bsg_view *gvp);
void ged_calc_adc_a2(struct bsg_view *gvp);
void ged_calc_adc_dst(struct bsg_view *gvp);

static void
adc_vls_print(struct bsg_view *gvp, fastf_t base2local, struct bu_vls *out_vp)
{
	...
	const char *argv[])
{
	...
	if (argc == 0) {
		...
		gedp->ged_gvp->gv_s->gv_adc.dv_a1 = (1.0 - (gedp->ged_gvp->gv_s->gv_adc.a1 / 45.0)) * BSG_VIEW_MAX;
	    }
	...
	if (argc == 0) {
		...
		gedp->ged_gvp->gv_s->gv_adc.dv_a2 = (1.0 - (gedp->ged_gvp->gv_s->gv_adc.a2 / 45.0)) * BSG_VIEW_MAX;
	    }
	...
	if (argc == 0) {
		...
		gedp->ged_gvp->gv_s->gv_adc.dv_dist = (gedp->ged_gvp->gv_s->gv_adc.dst / M_SQRT1_2 - 1.0) * BSG_VIEW_MAX;
	    }
	...
	if (argc == 1) {
		...
		adc_grid_to_adc_view(&(gedp->ged_gvp->gv_s->gv_adc), gedp->ged_gvp->gv_view2model, BSG_VIEW_MAX);
	...
	if (argc == 1) {
		...
		adc_model_to_adc_view(&(gedp->ged_gvp->gv_s->gv_adc), gedp->ged_gvp->gv_model2view, BSG_VIEW_MAX);
...
void
ged_calc_adc_pos(struct bsg_view *gvp)
{
    if (gvp->gv_s->gv_adc.anchor_pos == 1) {
	adc_model_to_adc_view(&(gvp->gv_s->gv_adc), gvp->gv_model2view, BSG_VIEW_MAX);
	adc_view_to_adc_grid(&(gvp->gv_s->gv_adc), gvp->gv_model2view);
    } else if (gvp->gv_s->gv_adc.anchor_pos == 2) {
	adc_grid_to_adc_view(&(gvp->gv_s->gv_adc), gvp->gv_view2model, BSG_VIEW_MAX);
...
void
ged_calc_adc_a1(struct bsg_view *gvp)
{
	...
	dx = view_pt[X] * BSG_VIEW_MAX - gvp->gv_s->gv_adc.dv_x;
	dy = view_pt[Y] * BSG_VIEW_MAX - gvp->gv_s->gv_adc.dv_y;

	if (!ZERO(dx) || !ZERO(dy)) {
	    gvp->gv_s->gv_adc.a1 = RAD2DEG*atan2(dy, dx);
	    gvp->gv_s->gv_adc.dv_a1 = (1.0 - (gvp->gv_s->gv_adc.a1 / 45.0)) * BSG_VIEW_MAX;
	}
    }
}
...
void
ged_calc_adc_a2(struct bsg_view *gvp)
{
	...
	if (!ZERO(dx) || !ZERO(dy)) {
	    ...
	    gvp->gv_s->gv_adc.dv_a2 = (1.0 - (gvp->gv_s->gv_adc.a2 / 45.0)) * BSG_VIEW_MAX;
	}
    }
}
...
void
ged_calc_adc_dst(struct bsg_view *gvp)
{
	...
	gvp->gv_s->gv_adc.dv_dist = (dist / M_SQRT1_2) - BSG_VIEW_MAX;
    } else
