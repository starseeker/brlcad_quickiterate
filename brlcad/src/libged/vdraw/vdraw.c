 ...
 * "c" is an integer representing one of the following bsg_vlist commands:
 *
 * BSG_VLIST_LINE_MOVE     0 / begin new line /
 * BSG_VLIST_LINE_DRAW     1 / draw line /
 * BSG_VLIST_POLY_START    2 / pt[] has surface normal /
 * BSG_VLIST_POLY_MOVE     3 / move to first poly vertex /
 * BSG_VLIST_POLY_DRAW     4 / subsequent poly vertex /
 * BSG_VLIST_POLY_END      5 / last vert (repeats 1st), draw poly /
 * BSG_VLIST_POLY_VERTNORM 6 / per-vertex normal, for interpolation /
 *
...
vdraw_write(void *data, int argc, const char *argv[])
{
    ...
    if (argv[2][0] == 'n') {
	/* next */
	for (REV_BU_LIST_FOR(vp, bsg_vlist, &(gedp->i->ged_gdp->gd_currVHead->vdc_vhd))) {
	    if (vp->nused > 0) {
		break;
	    }
	}
	if (BU_LIST_IS_HEAD(vp, &(gedp->i->ged_gdp->gd_currVHead->vdc_vhd))) {
	    ...
	    vp = BU_LIST_PNEXT(bsg_vlist, vp);
	    if (BU_LIST_IS_HEAD(vp, &(gedp->i->ged_gdp->gd_currVHead->vdc_vhd))) {
		BSG_GET_VLIST(vlfree, vp);
		BU_LIST_INSERT(&(gedp->i->ged_gdp->gd_currVHead->vdc_vhd), &(vp->l));
	    }
	}
	if (vp->nused >= BSG_VLIST_CHUNK) {
	    vp = BU_LIST_PNEXT(bsg_vlist, vp);
	    if (BU_LIST_IS_HEAD(vp, &(gedp->i->ged_gdp->gd_currVHead->vdc_vhd))) {
		BSG_GET_VLIST(vlfree, vp);
		BU_LIST_INSERT(&(gedp->i->ged_gdp->gd_currVHead->vdc_vhd), &(vp->l));
	...
	for (BU_LIST_FOR(vp, bsg_vlist, &(gedp->i->ged_gdp->gd_currVHead->vdc_vhd))) {
	    if ((size_t)uind < BSG_VLIST_CHUNK) {
...
vdraw_insert(void *data, int argc, const char *argv[])
{
    ...
    vp = BU_LIST_LAST(bsg_vlist, &(gedp->i->ged_gdp->gd_currVHead->vdc_vhd));
    vp->nused++;
	...
	wp = BU_LIST_PLAST(bsg_vlist, vp);
	vp->cmd[0] = wp->cmd[BSG_VLIST_CHUNK-1];
	VMOVE(vp->pt[0], wp->pt[BSG_VLIST_CHUNK-1]);
	vp = wp;
    }
...
vdraw_delete(void *data, int argc, const char *argv[])
{
    ...
    if ((size_t)uind >= vp->nused || uind >= BSG_VLIST_CHUNK) {
	bu_vls_printf(gedp->ged_result_str, "%s %s: delete out of range\n", argv[0], argv[1]);
	return BRLCAD_ERROR;
    }

    for (i = (size_t)uind; i < vp->nused - 1; i++) {
	if (i >= BSG_VLIST_CHUNK-1)
	    break;
	vp->cmd[i] = vp->cmd[i+1];
	VMOVE(vp->pt[i], vp->pt[i+1]);
    }

    wp = BU_LIST_PNEXT(bsg_vlist, vp);
    while (BU_LIST_NOT_HEAD(wp, &(gedp->i->ged_gdp->gd_currVHead->vdc_vhd))) {
	if (wp->nused == 0) {
	    break;
	}

	vp->cmd[BSG_VLIST_CHUNK-1] = wp->cmd[0];
	VMOVE(vp->pt[BSG_VLIST_CHUNK-1], wp->pt[0]);
...
vdraw_read(void *data, int argc, const char *argv[])
{
	...
	length = 0;
	vp = BU_LIST_FIRST(bsg_vlist, &(gedp->i->ged_gdp->gd_currVHead->vdc_vhd));
    ...
    /* The comparison to BSG_VLIST_CHUNK is to silence covarity */
    if ((size_t)uind >= vp->nused || uind >= BSG_VLIST_CHUNK) {
...
vdraw_vlist(void *data, int argc, const char *argv[])
{
	...
	case 'd':
	    ...
	    BSG_FREE_VLIST(vlfree, &(rcp2->vdc_vhd));
