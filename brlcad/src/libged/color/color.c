...
_edcolor(struct ged *gedp, int argc, const char *argv[])
{
    ...
    fprintf(fp, "%s", hdr);
    for (mp = rt_material_head(gedp->dbip); mp != MATER_NULL; mp = mp->mt_forw) {
	...
	while (rt_material_head(gedp->dbip) != MATER_NULL) {
	    zot = rt_material_head(gedp->dbip);
	    rt_new_material_head(gedp->dbip, zot->mt_forw);
...
ged_color_core(struct ged *gedp, int argc, const char *argv[])
{
	...
	rt_insert_color(gedp->dbip, newp);
	 ...
	 */
	rt_vls_color_map(&colors, gedp->dbip);
