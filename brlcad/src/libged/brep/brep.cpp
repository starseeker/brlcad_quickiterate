...
_brep_cmd_boolean(void *bs, int argc, const char **argv)
{
    ...
    GED_DB_GET_INTERNAL(gedp, &intern2, dp2, bn_mat_identity, BRLCAD_ERROR);
...
_brep_cmd_bots(void *bs, int argc, const char **argv)
{
	...
	GED_DB_GET_INTERNAL(gedp, &intern, dp, bn_mat_identity, BRLCAD_ERROR);
...
_brep_cmd_brep(void *bs, int argc, const char **argv)
{
	...
	struct rt_db_internal intern;
	GED_DB_GET_INTERNAL(gedp, &intern, gb->dp, bn_mat_identity, BRLCAD_ERROR);
	RT_CK_DB_INTERNAL(&intern);
...
_brep_cmd_selection(void *bs, int argc, const char **argv)
{
	...
	GED_DB_PUT_INTERNAL(gedp, gb->dp, &gb->intern, BRLCAD_ERROR);
    }
    return BRLCAD_OK;
}
...
ged_brep_core(struct ged *gedp, int argc, const char *argv[])
{
    ...
    GED_DB_GET_INTERNAL(gedp, &gb.intern, gb.dp, bn_mat_identity, BRLCAD_ERROR);
