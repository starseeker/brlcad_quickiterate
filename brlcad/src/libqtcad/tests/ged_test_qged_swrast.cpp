...
/* Callback for geometry changes, required when not using QgModel.
 * (In qged, QgModel handles this via Qt signals; here we do it ourselves.) */
extern "C" void
ged_changed_callback(struct db_i *UNUSED(dbip), struct directory *dp, int mode, void *u_data)
{
	...
	case 2:
	    ...
	    break;
	default:
	    bu_log("ged_changed_callback: unknown mode %d\n", mode);
	    break;
    }
}
...
main(int ac, char *av[])
{
    ...
    if (sw.displayManager()) {
        ...
        if (gr == 0 && dm_image) {
             ...
             * dm_get_display_image, but we still verified bsg_root and dmp. */
