...
QgAscImportDialog::QgAscImportDialog(QString filename, QString g_path, QString l_path)
{
	...
	connect(buttonBox, &QDialogButtonBox::accepted, this, &QgAscImportDialog::accept);
	connect(buttonBox, &QDialogButtonBox::rejected, this, &QgAscImportDialog::reject);
...
QgRhinoImportDialog::QgRhinoImportDialog(QString filename, QString g_path, QString l_path)
{
	input_file = filename;
	...
	connect(buttonBox, &QDialogButtonBox::accepted, this, &QgRhinoImportDialog::accept);
	connect(buttonBox, &QDialogButtonBox::rejected, this, &QgRhinoImportDialog::reject);
	QVBoxLayout *mainLayout = new QVBoxLayout;
	mainLayout->addWidget(formGroupBox);
	mainLayout->addWidget(buttonBox);
	setLayout(mainLayout);
...
QgStepImportDialog::QgStepImportDialog(QString filename, QString g_path, QString l_path)
{
	input_file = filename;
	...
	connect(buttonBox, &QDialogButtonBox::accepted, this, &QgStepImportDialog::accept);
	connect(buttonBox, &QDialogButtonBox::rejected, this, &QgStepImportDialog::reject);
	QVBoxLayout *mainLayout = new QVBoxLayout;
	mainLayout->addWidget(formGroupBox);
	mainLayout->addWidget(buttonBox);
	setLayout(mainLayout);
...
QgGeomImport::gfile(const char *tfile)
{
	...
	if (!fileinfo.suffix().compare("asc", Qt::CaseSensitive)) {
		QgAscImportDialog dialog(fileName, g_path, l_path);
		dialog.exec();
	...
	if (!fileinfo.suffix().compare("3dm", Qt::CaseInsensitive)) {
		QgRhinoImportDialog dialog(fileName, g_path, l_path);
		dialog.exec();
		g_path = dialog.db_path->text();
		l_path = dialog.log_path->text();
		exec_console_app_in_window(dialog.command(), dialog.options(), l_path);
	...
	if (!fileinfo.suffix().compare("stp", Qt::CaseInsensitive) || !fileinfo.suffix().compare("step", Qt::CaseInsensitive)) {
		QgStepImportDialog dialog(fileName, g_path, l_path);
		dialog.exec();
		g_path = dialog.db_path->text();
		l_path = dialog.log_path->text();
		exec_console_app_in_window(dialog.command(),dialog.options(), l_path);
