...
_bv_data_arrow_state_differ(struct bsg_data_arrow_state *v1, struct bsg_data_arrow_state *v2)
{
    ...
    BSG_NDIFF(1,gdas_draw);
    BSG_IVDIFF(1,gdas_color);
    BSG_NDIFF(1,gdas_line_width);
    BSG_NDIFF(1,gdas_tip_length);
    BSG_NDIFF(1,gdas_tip_width);
    BSG_NDIFF(1,gdas_num_points);
    // If we have the same number of points, check them
    for (int i = 0; i < v1->gdas_num_points; i++) {
	BSG_VDIFF(1,gdas_points[i]);
    }
    return 0;
}
...
_bv_data_axes_state_differ(struct bsg_data_axes_state *v1, struct bsg_data_axes_state *v2)
{
    ...
    BSG_NDIFF(1,line_width);
    BSG_NDIFF(1,size);
    BSG_NDIFF(1,num_points);
    // If we have the same number of points, check them
    for (int i = 0; i < v1->num_points; i++) {
	BSG_VDIFF(1,points[i]);
    }
    return 0;
}
...
_bv_data_label_state_differ(struct bsg_data_label_state *v1, struct bsg_data_label_state *v2)
{
    ...
    BSG_NDIFF(1,gdls_draw);
    BSG_IVDIFF(1,gdls_color);
    BSG_NDIFF(1,gdls_num_labels);
    // If we have the same number of labels, check them
    for (int i = 0; i < v1->gdls_num_labels; i++) {
	BSG_SDIFF(1,gdls_labels[i]);
    }
    BSG_NDIFF(1,gdls_size);
    // If we have the same number of points, check them
    for (int i = 0; i < v1->gdls_size; i++) {
	BSG_VDIFF(1,gdls_points[i]);
    }
    return 0;
}
...
_bv_data_line_state_differ(struct bsg_data_line_state *v1, struct bsg_data_line_state *v2)
{
    ...
    BSG_NDIFF(1,gdls_draw);
    BSG_IVDIFF(1,gdls_color);
    BSG_NDIFF(1,gdls_line_width);
    BSG_NDIFF(1,gdls_num_points);
    // If we have the same number of points, check them
...
/* First, do sanity checks */
    ...
    BSG_NDIFF(1,num_contours);
    BSG_IVDIFF(1,gp_color);
    BSG_NDIFF(1,gp_line_width);
    BSG_NDIFF(1,gp_line_style);
    for (size_t i = 0; i < v1->num_contours; i++) {
	BSG_CDIFF(1, _bg_poly_contour_differ, contour[i]);
    }
    if (v1->hole && v2->hole) {
	for (size_t i = 0; i < v1->num_contours; i++) {
	    BSG_NDIFF(1,hole[i]);
...
/* First, do sanity checks */
    ...
    BSG_NDIFF(1,num_polygons);
    for (size_t i = 0; i < v1->num_polygons; i++) {
	BSG_CDIFF(1, _bg_polygon_differ, polygon[i]);
    }
    return 0;
}
...
_bv_data_polygon_state_differ(bsg_data_polygon_state *v1, bsg_data_polygon_state *v2)
{
    ...
    BSG_NDIFF(1,gdps_draw);
    BSG_NDIFF(1,gdps_moveAll);
    BSG_IVDIFF(1,gdps_color);
    BSG_NDIFF(1,gdps_line_width);
    BSG_NDIFF(1,gdps_line_style);
    BSG_NDIFF(1,gdps_cflag);
    BSG_NDIFF(1,gdps_target_polygon_i);
    BSG_NDIFF(1,gdps_curr_polygon_i);
    BSG_NDIFF(1,gdps_curr_point_i);
    BSG_VDIFF(1,gdps_prev_point);
    BSG_DIFF(1,gdps_clip_type);
    BSG_NDIFF(1,gdps_scale);
    BSG_VDIFF(1,gdps_origin);
    BSG_MDIFF(1,gdps_rotation);
    BSG_MDIFF(1,gdps_view2model);
    BSG_MDIFF(1,gdps_model2view);
    BSG_NDIFF(1,gdps_data_vZ);
    BSG_CDIFF(1, _bg_polygons_differ, gdps_polygons);
...
_bv_settings_differ(struct bview_settings *v1, struct bview_settings *v2)
{
    ...
    BSG_NDIFF(1,adaptive_plot_mesh);
    BSG_NDIFF(1,adaptive_plot_csg);
    BSG_NDIFF(1,redraw_on_zoom);
    BSG_NDIFF(1,point_scale);
    BSG_NDIFF(1,curve_scale);
    BSG_NDIFF(1,bot_threshold);
...
int
bsg_view_differ(struct bview *v1, struct bview *v2)
{
    /* First, do sanity checks */
    if (!v1 && !v2)
	return -1;
    ...
    BSG_NDIFF(1, gv_tcl.gv_data_vZ);

    BSG_CDIFF(1, _bv_data_arrow_state_differ, gv_tcl.gv_data_arrows);
    BSG_CDIFF(1, _bv_data_axes_state_differ, gv_tcl.gv_data_axes);
    BSG_CDIFF(1, _bv_data_label_state_differ, gv_tcl.gv_data_labels);
    BSG_CDIFF(1, _bv_data_line_state_differ, gv_tcl.gv_data_lines);
    BSG_CDIFF(1, _bv_data_polygon_state_differ, gv_tcl.gv_data_polygons);
    BSG_CDIFF(1, _bv_data_arrow_state_differ, gv_tcl.gv_sdata_arrows);
    BSG_CDIFF(1, _bv_data_axes_state_differ, gv_tcl.gv_sdata_axes);
    BSG_CDIFF(1, _bv_data_label_state_differ, gv_tcl.gv_sdata_labels);
    BSG_CDIFF(1, _bv_data_line_state_differ, gv_tcl.gv_sdata_lines);
    BSG_CDIFF(1, _bv_data_polygon_state_differ, gv_tcl.gv_sdata_polygons);
    BSG_CDIFF(1, _bsg_other_state_differ, gv_tcl.gv_prim_labels);
