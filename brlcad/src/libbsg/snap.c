...
static int
_find_closest_obj_point(struct bv_cp_info *s, point_t *p, bsg_shape *o)
{
    int ret = 0;
    if (!s || !p || !o)
	return 0;
...
static double
line_tol_sq(bsg_view *v, int lwidth)
{
    if (!v || lwidth <= 0)
	return 100*100;
...
int
bsg_snap_lines_3d(point_t *out_pt, bsg_view *v, point_t *p)
{
    int ret = 0;
    struct bview_settings *gv_s = (v->gv_s) ? v->gv_s : &v->gv_ls;
...
int
bsg_snap_lines_2d(bsg_view *v, fastf_t *vx, fastf_t *vy)
{
    if (!v || !vx || !vy) return 0;
...
int
bsg_snap_grid_2d(bsg_view *v, fastf_t *vx, fastf_t *vy)
{
    point_t view_pt;
    point_t grid_origin;
