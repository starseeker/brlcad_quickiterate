		 ...
		 const char *path, bsg_draw_mode mode)
{
    ...
    g->s_flag = UP; /* visible by default */
...
test_revalidate_modified(struct bsg_view *v)
{
    ...
    CHECK(bsg_node_child_count(root) == 1, "MODIFIED: group not removed");
...
test_revalidate_removed(struct bsg_view *v)
{
    ...
    CHECK(bsg_node_child_count(root) == 1, "REMOVED: one group remains");

    bsg_node *remaining = bsg_node_child_at(root, 0);
...
test_revalidate_renamed(struct bsg_view *v)
{
    ...
    CHECK(bsg_node_child_count(root) == 1, "RENAMED: group not removed");
...
test_erase_by_path(struct bsg_view *v)
{
    ...
    CHECK(bsg_node_child_count(root) == 2,
...
test_simplify(struct bsg_view *v)
{
    ...
    CHECK(bsg_node_child_count(root2) == 2, "simplify: both kept when paths just share prefix string");
