 ...
 * information.
 */
/** @file libbsg/polygon_fill.cpp
 *
 * Slice 7 (bv_scene_obj_migrate):
 * bsg_polygon_fill_segments() — add internal fill lines to a polygon.
 *
 * Moved from libbv/polygon_fill.cpp.  This function has no dependency on
 * libbv types; it only needs libbg and libbu.  The implementation is
 * identical to the original; only the function name has changed from
 * bv_polygon_fill_segments to bsg_polygon_fill_segments.
 *
 * The old bv_polygon_fill_segments symbol is kept as a thin wrapper in
 * libbv for ABI compatibility during the migration period.
 */
			  ...
			  vect2d_t line_slope, fastf_t line_spacing)
{
    ...
    /* vp is non-const; no cast needed for bg_plane_* calls. */
    ...
    poly_2d.contour  = (struct bg_poly_contour *)bu_calloc(poly->num_contours,
	...
	poly_2d.contour[i].point = (point_t *)bu_calloc(poly->contour[i].num_points,
							 sizeof(point_t), "pc_point");
     ...
     * desired pattern.  Starting from the center of the bbox, construct line segments
     * parallel to line_slope that span the bbox and step perpendicular to line_slope
     * in both directions until the segments no longer intersect the bbox. */
    ...
    poly_lines.contour = (struct bg_poly_contour *)bu_calloc(poly_lines.num_contours,
							     sizeof(struct bg_poly_contour),
							     "p_contour");
    ...
    /* Step 1: positive perpendicular direction */
    ...
    /* Step 2: negative perpendicular direction */
    V2SET(per, lseg[1], -lseg[0]);
    V2UNITIZE(per);
    ...
    /* Clip fill lines to the 2-D polygon projection */
    struct bg_polygon *fpoly = bg_clip_polygon(bg_Intersection, &poly_lines, &poly_2d,
					       CLIPPER_MAX, NULL);
    ...
    /* Project result back to 3-D using the view plane */
    ...
    poly_fill->contour = (struct bg_poly_contour *)bu_calloc(fpoly->num_contours,
							     sizeof(struct bg_poly_contour),
							     "f_contour");
	...
	poly_fill->contour[i].point   = (point_t *)bu_calloc(fpoly->contour[i].num_points,
							     sizeof(point_t), "f_point");
	for (size_t j = 0; j < fpoly->contour[i].num_points; ++j) {
	    bg_plane_pt_at(&poly_fill->contour[i].point[j], vp,
