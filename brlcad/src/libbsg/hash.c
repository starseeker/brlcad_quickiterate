...
bsg_view_hash(struct bview *v)
{
    ...
    tbls[0] = bsg_view_shapes(v, BSG_DB_OBJS);
    tbls[1] = bsg_view_shapes(v, BSG_DB_OBJS | BSG_LOCAL_OBJS);
    tbls[2] = bsg_view_shapes(v, BSG_VIEW_OBJS);
    tbls[3] = bsg_view_shapes(v, BSG_VIEW_OBJS | BSG_LOCAL_OBJS);
    for (int t = 0; t < 4; t++) {
	if (!tbls[t])
	    continue;
