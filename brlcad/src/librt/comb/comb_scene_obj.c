...
/** @addtogroup librt */
/** @{ */
/** @file librt/comb/comb_scene_obj.c
 *
 * This module implements "evaluated" wireframe drawing for combination
 * objects: it takes seed nmg primitive wireframes and evaluates the CSG
 * booleans to produce more minimal wireframes — a.k.a drawing mode 3,
 * a.k.a the "bigE" mode originally exposed by the "E" command.
 *
 * Migrated from libged/wireframe_eval.c to librt so that the logic lives
 * in rt_comb_scene_obj() (ft_scene_obj for ID_COMBINATION), removing the
 * last draw_scene special-case from libged.
 *
 */
/** @} */
...
/* rt_comb_eval_m3 — evaluated wireframe (mode 3) for a combination.
 *
 * Migrated from draw_m3() in libged/wireframe_eval.c.  All access to
 * draw_update_data_t has been replaced with direct parameters:
 *   dbip  — from the ft_scene_obj dbip parameter
 *   ttol  — from the ft_scene_obj ttol parameter
 *   tol   — from the ft_scene_obj tol parameter
 * s->s_path and s->vlfree are accessed directly from bsg_shape (unchanged).
 */
int
rt_comb_eval_m3(bsg_shape *s,
		struct db_i *dbip,
		const struct bg_tess_tol *ttol,
		const struct bn_tol *tol)
{
