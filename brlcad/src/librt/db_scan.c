...
db_scan(struct db_i *dbip, int (*handler) (struct db_i *, const char *, b_off_t, size_t, int, void *), int do_old_matter, void *client_data /* argument for handler */)
{
    ...
    while (1) {
	nrec = 0;
	if (bu_fseek(dbip->i->dbi_fp, next, 0) != 0) {
	...
	switch (record.u_id) {
		...
		rt_memfree(&(dbip->i->dbi_freep), 1, addr/sizeof(union record));
		break;
	    case ID_ARS_A:
		while (1) {
		    here = bu_ftell(dbip->i->dbi_fp);
			...
			if (bu_fseek(dbip->i->dbi_fp, here, 0) == -1) {
		...
		for (; nrec < DB_SS_NGRAN; nrec++) {
		    if (fread((char *)&rec2, sizeof(rec2), 1, dbip->i->dbi_fp) != 1)
			break;
		}
		next = bu_ftell(dbip->i->dbi_fp);
		...
		if (do_old_matter) {
		    ...
		    rt_color_addrec(dbip, lo, hi, record.md.md_r, record.md.md_g, record.md.md_b, addr);
		}
		break;
	    case ID_P_HEAD:
		while (1) {
