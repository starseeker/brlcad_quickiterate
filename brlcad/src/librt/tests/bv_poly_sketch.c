...
test_non_origin_plane_roundtrip(void)
{
    ...
    struct bsg_node *pobj = bsg_create_polygon_obj(v, 0, p);
    /* bsg_create_polygon_obj initializes from the view; reset to the test plane. */
    bg_plane_pt_nrml(&p->vp, plane_pt, plane_n);
    bsg_polygon_vlist(pobj);
    ...
    struct bsg_node *rtobj = db_sketch_to_scene_obj("roundtrip", wfp->dbip, odp, v, 0);
    if (!rtobj)
...
main(int argc, char *argv[])
{
    ...
    struct bsg_node *opobj = db_sketch_to_scene_obj("poly_out", wfp->dbip, odp, v, 0);
    if (!opobj)
