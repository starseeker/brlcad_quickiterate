...
int
rt_extrude_plot(struct bu_list *vhead, struct rt_db_internal *ip, const struct bg_tess_tol *ttol, const struct bn_tol *UNUSED(tol), const bsg_view *UNUSED(info))
{
    ...
    struct bsg_vlist *vp1, *vp2, *vp2_start;
	...
	BSG_ADD_VLIST(vlfree, vhead, extrude_ip->V, BSG_VLIST_LINE_MOVE);
	BSG_ADD_VLIST(vlfree, vhead, extrude_ip->V, BSG_VLIST_LINE_DRAW);
	return 0;
    }
    ...
    /* plot bottom curve */
    vp1 = BU_LIST_LAST(bsg_vlist, vhead);
    nused1 = vp1->nused;
    ...
    vp2 = BU_LIST_LAST(bsg_vlist, vhead);
    nused2 = vp2->nused;
    ...
    if (i1 >= vp1->nused) {
	i1 = 0;
	vp1 = BU_LIST_NEXT(bsg_vlist, &vp1->l);
    }
    i2 = nused2;
    if (i2 >= vp2->nused) {
	i2 = 0;
	vp2 = BU_LIST_NEXT(bsg_vlist, &vp2->l);
	nused2--;
    }

    while (vp1 != vp2_start || (i1 < BSG_VLIST_CHUNK && i2 < BSG_VLIST_CHUNK && i1 != nused2)) {
	BSG_ADD_VLIST(vlfree, vhead, vp1->pt[i1], BSG_VLIST_LINE_MOVE);
	BSG_ADD_VLIST(vlfree, vhead, vp2->pt[i2], BSG_VLIST_LINE_DRAW);
	i1++;
	if (i1 >= vp1->nused) {
	    i1 = 0;
	    vp1 = BU_LIST_NEXT(bsg_vlist, &vp1->l);
