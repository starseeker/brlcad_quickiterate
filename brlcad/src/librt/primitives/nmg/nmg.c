...
nmg_stash_model_to_file(const char *filename, const struct model *m, const char *title)
{
	...
	if (rt_db_cvt_to_external5(&ext, name, &intern, 1.0, fp->dbip, intern.idb_major_type) < 0) {
 ...
 * curtree = rt_booltree_evaluate(curtree, vlfree, tol, &rt_nmg_do_bool, 0, NULL);
 */
int
rt_nmg_do_bool(
...
{
    return rt_booltree_evaluate(tp, vlfree, tol, &rt_nmg_do_bool, nmg_bool_eval_silent, NULL);
}
...
nmg_boolean(union tree *tp, struct model *m, struct bu_list *vlfree, const struct bn_tol *tol)
{
     ...
     */
    result = rt_booltree_evaluate(tp, vlfree, tol, &rt_nmg_do_bool, nmg_bool_eval_silent, NULL);
