...
db_scene_obj_to_sketch(struct db_i *dbip, const char *sname, struct bv_scene_obj *s)
{
    ...
    point_t u_basis_end, v_basis_end;
    bg_plane_pt_at(&u_basis_end, &p->vp, 1, 0);
    bg_plane_pt_at(&v_basis_end, &p->vp, 0, 1);
    VSUB2(sketch_ip->u_vec, u_basis_end, sketch_ip->V);
    VSUB2(sketch_ip->v_vec, v_basis_end, sketch_ip->V);
