...
db_scene_obj_to_sketch(struct db_i *dbip, const char *sname, struct bv_scene_obj *s)
{
    ...
    point_t u_end_pt, v_end_pt;
    bg_plane_pt_at(&u_end_pt, &p->vp, 1, 0);
    bg_plane_pt_at(&v_end_pt, &p->vp, 0, 1);
    VSUB2(sketch_ip->u_vec, u_end_pt, sketch_ip->V);
    VSUB2(sketch_ip->v_vec, v_end_pt, sketch_ip->V);
