...
int
rt_ell_adaptive_plot(struct bu_list *vhead, struct rt_db_internal *ip, const struct bn_tol *tol, const struct bsg_view *v, fastf_t s_size)
{
	...
	BSG_ADD_VLIST(vlfree, vhead, eip->v, BSG_VLIST_POINT_DRAW);
	return 0;
    }
...
int
rt_ell_plot(struct bu_list *vhead, struct rt_db_internal *ip, const struct bg_tess_tol *UNUSED(ttol), const struct bn_tol *UNUSED(tol), const struct bsg_view *UNUSED(info))
{
    register int i;
    struct rt_ell_internal *eip;
    fastf_t top[16*3];
