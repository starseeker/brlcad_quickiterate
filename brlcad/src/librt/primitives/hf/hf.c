...
int
rt_hf_plot(struct bu_list *vhead, struct rt_db_internal *ip, const struct bg_tess_tol *ttol, const struct bn_tol *UNUSED(tol), const struct bsg_view *UNUSED(info))
{
    ...
    BSG_ADD_VLIST(vlfree, vhead, xip->v, BSG_VLIST_LINE_MOVE);

    VJOIN1(start, xip->v, xip->xlen, xip->x);
    BSG_ADD_VLIST(vlfree, vhead, start, BSG_VLIST_LINE_DRAW);
    ...
    BSG_ADD_VLIST(vlfree, vhead, xip->v, BSG_VLIST_LINE_DRAW);
    goal -= 5;
...
#define HF_GET(_p, _x, _y)	((_p)[(_y)*xip->w+(_x)])
	...
	for (x = 0; x < xip->w; x++) {
	    ...
	    BSG_ADD_VLIST(vlfree, vhead, cur, BSG_VLIST_LINE_DRAW);
	    sp++;
	}
	...
	VJOIN1(start, xip->v, xip->ylen, xip->y);
	BSG_ADD_VLIST(vlfree, vhead, start, BSG_VLIST_LINE_MOVE);
	...
	/* Y direction, X=0 */
	cmd = BSG_VLIST_LINE_MOVE;
	sp = &HF_GET((unsigned short *)xip->mp->apbuf, 0, 0);
	for (y = 0; y < xip->n; y++) {
	    VJOIN2(cur, xip->v, y, ybasis, *sp, zbasis);
	    BSG_ADD_VLIST(vlfree, vhead, cur, cmd);
	    cmd = BSG_VLIST_LINE_DRAW;
	    sp += xip->w;
	}
	...
	if (xip->shorts) {
	    ...
	    VJOIN2(cur, start, x, xbasis, *sp, zbasis);
	    BSG_ADD_VLIST(vlfree, vhead, cur, BSG_VLIST_LINE_MOVE);
	    x += half_step;
	    sp = &HF_GET((unsigned short *)xip->mp->apbuf, x, y);
	    for (; x < xip->w; x += step) {
		VJOIN2(cur, start, x, xbasis, *sp, zbasis);
