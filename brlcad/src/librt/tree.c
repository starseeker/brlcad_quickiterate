			 ...
			 struct resource *resp)
{
	...
	case OP_XOR: {
	    ...
	    left  = rt_tree_prune_subtractor(tp->tr_b.tb_left,  cmin, cmax, resp);
	    right = rt_tree_prune_subtractor(tp->tr_b.tb_right, cmin, cmax, resp);
	    if (!left && !right) {
 ...
 */
static union tree *
rt_tree_shake_subs(union tree *tp, struct resource *resp)
{
	...
	case OP_SUBTRACT:
	    ...
	    if (tp->tr_b.tb_left)
		tp->tr_b.tb_left  = rt_tree_shake_subs(tp->tr_b.tb_left,  resp);
	    if (tp->tr_b.tb_right)
		tp->tr_b.tb_right = rt_tree_shake_subs(tp->tr_b.tb_right, resp);
		...
		tp->tr_b.tb_right, left_min, left_max, resp);
