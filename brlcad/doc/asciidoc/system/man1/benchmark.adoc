...
There are various _command_s that may be give to the BRL-CAD benchmark that cause it to perform various actions such as invoking the computation tests or filesystem clean-up:
...
_benchmark run DEVIATION=1 TIMEFRAME=60 MAXTIME=600_ excessive analysis, attempt to stabilize within 1 percent deviation with each frame taking at least 60 seconds but no more than 10 minutes per test (the entire analysis will probably take 30 to 60 minutes) _benchmark run QUIET=1 -P1_ perform a benchmark analysis only using one CPU and only logging results to a file
