...
[source]
----
gdiff -G [<grouping_options>]... [<pattern>]...
----
...
*gdiff* in its basic mode compares BRL-CAD models specified on the command line, reporting differences according to the specified options. It supports a plain "diff" comparison between two files, as well as a "3-way" comparison that accepts a third file with "ancestral" information that provides a context for the left and right files.

In addition to direct comparison, *gdiff* supports a similarity grouping mode (*-G*) that analyzes one or more BRL-CAD database files and clusters them into groups of related models based on _per-file content similarity_ (and optionally on filename similarity for an extra top-level binning pass.) Grouping mode produces a human-readable report showing groups, a representative "newest" file, distances to a computed group center (medoid), and modification dates.

The following options are recognized in basic diffing mode:
...
*-q*::
Decrease the verbosity of the output. Additional specifications will further decrease the output provided. (Opposite effect of *v* option.) All settings for this option will respect other constraints applied to the reporting output.
...
*-v*::
Increase the verbosity of the output. Additional specifications will further increase the output provided. (Opposite effect of *q* option.) All settings for this option will respect other constraints applied to the reporting output.
...
In grouping mode, *gdiff -G* does not compare two databases. Instead, it scans one or more directories and/or file paths, identifies candidate `.g` files (by pattern), computes a content hash for each file using _TLSH_, and clusters the files by content similarity.

[[grouping-algorithm]]
=== Algorithm overview


* *Content hashing:* For each .g file, a single TLSH digest is computed over selected inputs: (a) a stable sequence of object names (if enabled), and/or (b) geometry data (if enabled). Geometry can be hashed in a _deep_ mode (similarity-sensitive) or a _fast_ mode (robust but treats any change as maximally different per-object, then aggregates those integers deterministically.)
* *Similarity graph:* All files form vertices in a graph. An undirected edge is added between vertices i and j if the TLSH distance between their hashes satisfies the threshold condition (`diff < threshold`). TLSH is not a metric (triangle inequality is not guaranteed), so direct metric trees are not used for clustering.
* *Clustering:* Clusters are the connected components of the similarity graph (transitive closure). This captures families like A~B~C even if A and C are not directly similar.
* *Representative files:* Within each cluster, the _newest_ file (by filesystem mtime) is used as the top-level label. A _center_ (medoid) is also identified as the file minimizing total TLSH distance to the other cluster members.
* *Optional filename binning:* If enabled, files are first binned by base-name similarity using a _BK-tree_ and Damerau-Levenshtein edit distance. Clustering then runs independently within each filename bin.


[[grouping-report]]
=== Report format

Grouping mode prints a one-time header explaining the report, then one or more groups. For each group:


* Top-level (newest) line shows the file path and date, followed by a bracketed score which is the TLSH distance from the _newest_ file to the _group center_ (medoid). Top-level lines are column-aligned across groups.
* Member lines (indented) list each group file (excluding the newest), the TLSH distance of each file to the newest, and the TLSH distance to the group center. Dates are YYYY-MM-DD.
* Groups (top-level newest lines) are ordered with an _alphanumeric_ comparator (natural sort) by displayed path, so "file2" sorts before "file10".


[[grouping-options]]
=== Options (grouping mode)


*-G*, *--group*::
Enable grouping mode - this is needed to trigger grouping mode and enable the other related options. Arguments are treated as directories and/or file paths to scan; patterns may be supplied to constrain discovery (see *-P*).


*--filename-threshold* _#_::
Enable an initial filename binning pass using Damerau-Levenshtein edit distance less than or equal to _#_. Files whose base names are within this threshold are grouped into a filename bin; content clustering runs within each bin. Default: disabled.


*--threshold* _#_::
Content similarity threshold for TLSH distances. An undirected similarity edge is added between two files when `diff < #`. Larger values yield looser clustering (more files per group). Default: 30.

In normal usage this is the value users will most likely want to adjust. (For large input sets it is _highly_ recommended that hashes be stored and reused with the *--write-hashes* and *--read-hashes* options to save processing time.) Lower and higher values will result in more and less agressive bin creation - the user will need to develop an intuitive feel for what settings produce the most useful groups for their particular inputs.


*--use-objnames*::
Include sorted object names (with a seed) when building the content hash. On by default if neither *--use-objnames* or *--use-geometry* is specified - if the other is specified, off unless enabled explicitly.


*--use-geometry*::
Include geometry data when building the content hash. On by default if neither *--use-objnames* or *--use-geometry* is specified - if the other is specified, off unless enabled explicitly.


*--fast-geometry*::
Use a faster but more sensitive geometry hashing mode. Any geometric change within an object is treated as maximally different at the object level; file-level aggregation is stable but may report larger distances. Useful for speed at the expense of similarity sensitivity. Off by default.


*-P*, *--file-pattern* _pattern_::
Recursive discovery pattern for selecting .g files under directories. Default: `*.g`. Multiple patterns may be supplied. Pattern semantics follow shell-style matching.


*--read-hashes* _file_::
Read a cache file of precomputed hashes and reuse matching entries when possible. The cache is versioned and entries are validated against current file size and mtime.


*--write-hashes* _file_::
Write all valid in-memory hashes to a cache file in a versioned CSV format for reuse in subsequent runs.


*--display-paths* _auto|relative|absolute_::
Controls how file paths are shown in reports. `auto` displays relative paths when all input arguments are relative; otherwise absolute. `relative` enforces relative, `absolute` enforces absolute. Default: `auto`.


*-v*, *-q*::
Verbosity controls apply to grouping mode as well. With verbosity enabled, *gdiff -G* prints a one-time summary before hashing (e.g., number of discovered files, how many will be hashed vs. read from cache) and logs each hashed file with elapsed time in seconds: `[hash] path/to/file.g 1.234s`.
...
[example]
====
The *v* and *q* options provide various styles of output, as follows:

*-qq* (no output except the return code):
...
[example]
====
The _search_ filtering technique can be used to select specific subsets of the database to see in reports, using the *F* option. For example, limiting the diff report to only objects of type elliptical torus:
...
.GROUPING MODE - Basic Grouping by Content Similarity (-G)
[example]
====
A basic example of grouping mode output (note that the actual output will utilize Unicode characters - ascii approximations are used here for broadest display support.)

In the example below, you can see two major groupings identified as well as a couple miscellaneous files that don't meet the threshold for either group. The filenames indicate that at the default threshold value, the major determinator for which group a file goes in is whether it is a triangle (BoT) conversion or an unfacetized original, which is one of the grouping criteria we would expect for content-based grouping analysis.
...
~: gdiff -G /Models
[group] Discovered 28 .g file(s). 28 to hash, 0 from cache.
====================== gdiff -G Grouping Summary ======================
Hash inputs: names+geometry
Filename binning inactive: all files clustered together by content similarity.
Distance thresholds (lower value = greater similarity): TLSH < 30
Columns:
  Seed/Newest line: [dx####] distance from newest file to group center.
  Member lines: [d####, dx####] distances to newest and to group center.
=======================================================================

/Models/Dir1/model_2018-01-12.g         [dx   6]        2018-01-12
  /Models/Dir1/model_2017_03_14_bot.g   [d   9, dx  10] 2017-03-14
  /Models/Dir1/model_tri.g              [d   2, dx   7] 2015-08-25
  /Models/Dir2/model_031215_bot.g       [d   2, dx   5] 2015-03-12
  /Models/Dir2/model_2015_01_27_bot.g   [d   6, dx   1] 2015-01-27
  /Models/Dir2/model_011415_bot.g       [d   6, dx   1] 2015-01-16
  /Models/Dir1/model_111314_bot.g       [d   6, dx   1] 2014-11-13
  /Models/Dir1/model_110614_bot.g       [d   3, dx   6] 2014-11-06
  /Models/Dir1/model_032314_bot.g       [d   6, dx   0] 2014-03-23
  /Models/Dir2/model_021014_bot.g       [d   7, dx   6] 2014-02-25
  /Models/Dir2/model_020214_bot.g       [d  22, dx  21] 2014-02-02
  /Models/Dir2/model_012614_bot.g       [d  22, dx  21] 2014-01-26
  /Models/Dir2/model_011614_bot.g       [d  22, dx  21] 2014-01-16
/Models/Dir2/model_011415.g             [dx   0]        2015-01-14
  /Models/Dir1/model_121318.g           [d   1, dx   1] 2014-12-18
  /Models/Dir1/model_112214.g           [d   1, dx   1] 2014-12-14
  /Models/Dir1/model_072114.g           [d   2, dx   2] 2014-07-21
  /Models/Dir2/model_061014.g           [d   3, dx   3] 2014-06-25
  /Models/Dir2/model_060114.g           [d  15, dx  15] 2014-06-02
  /Models/Dir2/model_042314.g           [d  15, dx  15] 2014-04-25
  /Models/Dir2/orig_042014.g            [d  15, dx  15] 2014-04-20
  /Models/Dir2/model_041814.g           [d  15, dx  15] 2014-04-18
  /Models/Dir2/orig_041414.g            [d  15, dx  15] 2014-04-15
  /Models/Dir2/orig_040414.g            [d  16, dx  16] 2014-04-09
  /Models/Dir2/model_040214.g           [d  16, dx  16] 2014-04-03
  /Models/Dir2/orig_040114.g            [d  16, dx  16] 2014-04-01
/Models/Dir2/misc.g                     [dx   0]        2014-02-29
/Models/Dir2/c.g                        [dx   0]        2014-04-15
...
This software is Copyright (c) 2014-2025 by the United States Government as represented by U.S. Army Research Laboratory.
