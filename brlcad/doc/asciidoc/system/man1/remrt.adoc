...
*remrt* depends on the Berkeley _rsh (1)_ remote shell command and the user's corresponding ".rhosts" file to automatically initiate remote computation.
...
with a "when" value of _passive_ and a "where" value of _convert_. When an _rtsrv_ passes from night into day, it is automatically terminated by *remrt* at the appointed time.
...
*rtsrv*(1), *rt*(1), *scriptsort*(1), *brlcad*(1), *mged*(1), *pix-fb*(1), *rtray*(1), *rtpp*(1), *librt*(3), *ray*(5V), *pix*(5)
