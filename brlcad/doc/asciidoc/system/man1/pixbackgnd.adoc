...
By default, the title height is 80 scanlines, or 90 scanlines if the *-H* flag is set, but the *-t*_title_height_ option may be used to specify another value. If _title_height_ is set to zero, then the white bar is omitted as well, and the entire display is smoothly shaded.
