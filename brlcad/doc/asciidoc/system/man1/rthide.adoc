...
*-U #*::
sets the Boolean variable _use_air_ to the given value.
...
The plotfile resulting from an *rthide* run can be viewed directly via _plot3-fb_ and may be rotated with _plot3rot_ at the user's option:
...
The output of *rthide* can be overlaid on top of a _rt_ shaded image, for "edge enhancement". Here is a complete example, which assumes that FB_FILE points to a valid framebuffer:
...
This software is Copyright (c) 1990-2022 by the United States Government as represented by U.S. Army Research Laboratory.
