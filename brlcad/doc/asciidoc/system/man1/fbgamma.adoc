...
where gamma will usually lie in the 2.0 to 3.0 range (2.2 is the _NTSC_ standard gamma, 2.5 is a commonly used "typical" value). Most of the time, pixel values of images have been computed assuming a linear mapping between pixel value and displayed intensity. This linear mapping can be approximated by loading a colormap into the display which raises the pixel values to the inverse gamma power.
...
This software is Copyright (c) 1987-2025 by the United States Government as represented by U.S. Army Research Laboratory.
