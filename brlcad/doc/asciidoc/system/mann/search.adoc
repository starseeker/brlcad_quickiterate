...
|/objname |Use object _objname_ as the starting point, return list of matching full paths. 
|objname |Use object _objname_ as the starting point, return list of all matching objects. 
||/objname |Use every object below _objname_ as starting points, return list of matching full paths. 
||objname |Use every object below _objname_ as starting points, return list of all matching objects. 
|===
...
|grip |V, N, L 
|half |N, d 
|hf |cfile, dfile, fmt, w, n, shorts, file2mm, v, x, y, xlen, ylen, zscale 
|hyp |V, H, A, b, bnr 
|part |V, H, r_v, r_h 
...
*-stdattr*::
Returns TRUE if an object has ONLY standard attributes associated with it. Standard attributes in BRL-CAD are: .Standard Attributes
|===
...
*-type*_pattern_::
Returns TRUE if the type of the object matches _pattern._ Pattern evaluation is done using shell pattern matching. Types recognized include: .Primitives
|===
...
As a convenience, the *-type* option will also recognize the pattern _shape_, which will match any object that is _not_ a combination, region, annotation, constraint, grip or joint.
...
....
	  ...
	  mged> search all.g -type region -and ( -below -bool - -or -below -bool + ) -exec draw "{}" ";"mged> autoview
...
....
	  ...
	  mged>  set glob_compat_mode 0mged>  set search_results [search cab -type region]
