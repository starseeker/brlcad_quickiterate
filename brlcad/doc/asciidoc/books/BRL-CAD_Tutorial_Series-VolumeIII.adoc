...
+
For articulation and animation, this generally means that objects that normally move together (e.g., components on a helicopter rotor, tank turret, etc.) should be grouped together in assembly combinations (as shown in Figure 15). .Example of grouping objects for articulation.
...
....
 ...
 Diagrammed, this line breaks down as follows: [cols="4*"]
[%noheader]
|===
|proc
|256wa
...
The diagrammed command for sizing and positioning the command window is as follows: [cols="4*"]
[%noheader]
|===
|set mged_default
|(geom)
