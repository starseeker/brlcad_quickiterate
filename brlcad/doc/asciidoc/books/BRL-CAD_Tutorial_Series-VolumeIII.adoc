...
image::../books/images/tutorial_series_volIII_fig01.png[width=80%]

image::../books/images/tutorial_series_volIII_fig02.png[width=4in]

image::../books/images/tutorial_series_volIII_fig02.png[width=80%]
...
image::../books/images/tutorial_series_volIII_fig30.png[width=1in]

image::../books/images/tutorial_series_volIII_fig30.png[]
...
image::../books/images/tutorial_series_volIII_fig21.png[width=1in]

image::../books/images/tutorial_series_volIII_fig21.png[]



 |Can arguably provide the best source of verifiable information by providing hands-on access to the actual objects being modeled. |Can be resource and labor intensive and can be limited by the objects’ availability, accessibility, and measurability, resulting in missed measurements. 
|Converted geometry

image::../books/images/tutorial_series_volIII_fig22.png[width=1in]

image::../books/images/tutorial_series_volIII_fig22.png[]



 |Can offer significant savings in data collection and/or measurement efforts. |Can have missing data, unfamiliar naming schemes, and alternate/dissimilar geometry formats (e.g., feature-based objects, splines, etc.). Can also be unsuitable for a given application because the original model was developed for a different purpose. 
|===
...
....
 ...
 |Associates the name with function (front face), MGED object type and order of creation (.s1), and type of Boolean operation performed (subtraction [−] and intersection [+]).footnote:[Note that we have chosen not to associate a suffix for objects that are unioned. In this naming convention, objects without a Boolean operation suffix are understood to be unioned.
...
|Primitive Shape/BRL-CAD Abbreviation |Input Parameters/Definitionsfootnote:[To maximize database efficiency, some shape types are stored as other types (e.g., all arbs are stored as arb8's), but this behavior is invisible to the user.
...
+
image::../books/images/tutorial_series_volIII_fig23.png[]
...
|cp |Creates a duplicate of a previously defined object. |Copies the parameters of an object to a new object of the same type. Takes advantage of previously defined measurements and locations. |Command line 
...
|mirror |Creates a duplicate primitive shape, region, or assembly and locates it across the x axis, y axis, z axis, or an arbitrary axis specified by a point and a direction. |Takes advantage of previously defined measurements and locations; can mirror across only one axis at a time but across any point along that axis. |Command line 
...
+
For articulation and animation, this generally means that objects that normally move together (e.g., components on a helicopter rotor, tank turret, etc.) should be grouped together in assembly combinations (as shown in Figure 15). image::../books/images/tutorial_series_volIII_fig15.png[]
...
** _comb_ - creates a combination using Boolean expressions in GIFT footnote:[Geometric information for targets (GIFT) is the single-level operator hierarchy format that is the traditional (and default) notation used in BRL-CAD.

] format. Proceeding left to right, intersections (+) and subtractions (") are performed before unions (u). For example, the command 
....
...
** _g_creates a combination by automatically unioning all user-specified elements together. Thus, this command does not accept any sort of Boolean operators from the user.footnote:[The *g* command is derived from “group,” the term sometimes used for _assembly combination_.
...
|Medium forest green |107 142 50 
...
|Armament (not ammunition)footnote:[armygreenrecommended] |Gray |80 80 80 
|Propellant |Magenta |255 0 255 
|Projectiles |Red |255 0 0 
|Engine/propulsionfootnote:[armygreenrecommended] |Blue |0 0 255 
...
|Driver/flight controlsfootnote:[armygreenrecommended] |Dark blue |50 0 175 
|Suspension/rotor bladesfootnote:[armygreenrecommended] |Gray |80 80 80 
...
|- mged>p 500 -500 150 |image::../articles/images/pipes_fig05.png[]

image::../articles/images/pipes_fig05.png[]

 
|- mged>p -500 -500 250 |image::../articles/images/pipes_fig06.png[]

image::../articles/images/pipes_fig06.png[]

 
|- mged>p -500 500 350 |image::../articles/images/pipes_fig07.png[]

image::../articles/images/pipes_fig07.png[]

 
|- mged>p 500 500 450 |image::../articles/images/pipes_fig08.png[]

image::../articles/images/pipes_fig08.png[]

 
|- mged>p 500 -500 550 |image::../articles/images/pipes_fig09.png[]

image::../articles/images/pipes_fig09.png[]

 
|- mged>p -500 -500 650 |image::../articles/images/pipes_fig10.png[]

image::../articles/images/pipes_fig10.png[]

 
|- mged>p -500 500 750 |image::../articles/images/pipes_fig11.png[]

image::../articles/images/pipes_fig11.png[]

 
|- mged>p 0 500 800 |image::../articles/images/pipes_fig12.png[]
...
|proc |256wa |{} |{rt -s256 -C255/255/255 -A.7} 
|Denotes that a Tcl procedure is being created. |Names the procedure 256wa. |Denotes that there are no arguments. |Denotes the MGED command that will be executed. The rendering will be 256 pixels square size (as signified by -s), will have a background red-greenblue value of 255 255 255 (as signified by the -C option), and will have an ambient light setting of 0.7 (as signified by the -A option). 
|===
...
image::../articles/images/build_pattern_fig07.png[]
