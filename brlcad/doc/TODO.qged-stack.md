...
**P2 — `loadview.cpp` `_ged_cm_vsize` camera field direct writes**
`src/libged/draw/loadview.cpp` lines 49–51 write `gv_size`, `gv_scale`, and
`gv_isize` directly with no `bsg_view_set_size()` accessor available yet.
Fix: add `bsg_view_set_size(v, size)` to `bsg/util.h` + `scene_graph.cpp`, then
migrate `_ged_cm_vsize` to use it.

**P2 — `src/libdm/swrast/fb-swrast.cpp` scene-root initialisation**
The software-rasterizer framebuffer backend does not call `bsg_scene_root_create()`
at view initialization.  Straightforward fix following the same pattern as
`mged/attach.c`.

**P2 — `mged/cmd.c` `_view_cache` struct uses raw `gv_aet`/`gv_rotation`**
The view-preset cache copies raw camera fields instead of using `bsg_camera`.
Low-risk since MGED is considered in maintenance mode, but a clean migration
follows the `bsg_view_get/set_camera()` pattern.
...
| Item | File(s) | Notes | Effort |
|------|---------|-------|--------|
| `QgConsoleListener` — verify thread safety of `ged_result_str` access | `QgConsoleListener.cpp` | Comments state "confined to main thread" — audit and add assertion or mutex | 2 h |
...
> The `ged_result_str` data race requires an audit to confirm the "main-thread only"
> comment is actually enforced by the Qt::QueuedConnection plumbing.

### Tier 2 — Safety and maintenance

| Item | File(s) | Effort |
|------|---------|--------|
| Add `override` to virtual overrides in `qged/` and `libqtcad/` | Many headers | 2 h |
| Move `raytrace.h`-dependent sketch export out of `libqtcad` into `qged` | `QgView.cpp`, `QgPolyFilter.cpp`, `QgSelectFilter.cpp`, `QgMeasureFilter.cpp` | 4 h |
| Migrate `fbserv.cpp` from `QTcpServer`/`QTcpSocket` to `QLocalServer`/`QLocalSocket` | `fbserv.cpp/.h` | 4 h |
| Convert `QgModel::rootItem` / `items` (raw pointer forest) to `unique_ptr` ownership | `QgModel.cpp/.h` | 4 h |
| Consolidate view-update signal chain (`do_view_changed` → `view_update`) to eliminate reentrancy risk | `QgEdMainWindow.cpp:252` | 2 h |

### Tier 3 — C++17 / Qt modernisation

| Item | Effort |
|------|--------|
| `[[nodiscard]]` on `run_cmd()` and other error-returning functions | 1 h |
| Implement `QgFlowLayout` Qt6 geometry path (currently stub) | 2 h |
| Implement `QgConsole` mid-string tab completion for Qt6 | 3 h |
| Apply `[[maybe_unused]]` to replace `UNUSED()` macro calls | 1 h |
...
### Short-term (correctness / safety — do first)

1. **Audit `QgConsoleListener` thread safety** (Tier 1): confirm that
   `Qt::QueuedConnection` on `readyRead` actually serializes `ged_result_str` access
   and that no path writes to it from the I/O thread without going through the queue.
   Add a `Q_ASSERT(QThread::currentThread() == qApp->thread())` guard inside the
   slot if not already present.

2. **`raytrace.h` out of `libqtcad`** (Tier 2): move the sketch polygon export
   function to `qged` so `libqtcad` no longer pulls in all of `librt` transitively.
   This reduces the dependency footprint and makes `libqtcad` usable in lighter
   contexts.

3. **`fbserv.cpp` → `QLocalSocket`** (Tier 2): the existing `QTcpServer` approach
   binds to localhost:0 and is vulnerable to local port scanning.  `QLocalSocket`
   (Unix domain socket / Windows named pipe) is both faster and more secure for
   in-process framebuffer communication.

### Medium-term (modernisation)

4. **Add `override` throughout** (Tier 2 / mechanical): prevents silent virtual
   dispatch breakage when base class signatures change.

5. **`QgModel` ownership** (Tier 2): convert `rootItem` and the `items` set to
   `unique_ptr`-based ownership to eliminate the raw-pointer forest.

6. **View update consolidation** (Tier 2): wrap the `do_view_changed` signal chain
   in a `QMetaObject::invokeMethod(..., Qt::QueuedConnection)` coalescing wrapper to
   eliminate the reentrancy risk noted at `QgEdMainWindow.cpp:252`.

### Longer-term (features)

7. **AABB placeholder rendering** (Section 7.4 above): natural follow-on to
   `GeomLoader`; straightforward given the current bbox cache.

8. **Primitive editing** (Tier 4): the largest single feature gap.  The design
   (Section 7.1) is clear; implementation is substantial but incremental.

9. **BSG P2 items**: `bsg_view_set_size()` accessor for loadview; swrast scene-root
   init; mged `_view_cache` camera migration.  Low risk, address opportunistically.
