...
void PrintMetrics(std::string const& label, QualityMetrics const& metrics)
{
    ...
    std::cout << "  Edge: avg=" << metrics.avgEdgeLength 
              << ", stddev=" << metrics.edgeLengthStdDev << std::endl;
    std::cout << "  Quality: avg=" << metrics.avgTriangleQuality
              << ", min=" << metrics.minTriangleQuality << std::endl;
}

int main()
{
    std::cout << "===== Newton/BFGS CVT Optimizer Test =====" << std::endl;

    // Load test mesh
    std::vector<Vector3<double>> vertices;
    std::vector<std::array<int32_t, 3>> triangles;

    std::vector<std::string> testFiles = { "test_tiny.obj", "stress_concave.obj" };
    bool loaded = false;
    ...
    auto initialMetrics = ComputeQuality(vertices, triangles);
    PrintMetrics("\nInitial Mesh", initialMetrics);

    // Test 1: Lloyd only (RVD-based)
    std::cout << "\n===== Test 1: Lloyd Only (5 iterations) =====" << std::endl;
    {
        ...
        if (MeshRemesh<double>::Remesh(testVertices, testTriangles, params))
        {
            ...
            std::cout << "SUCCESS: Lloyd remeshing complete" << std::endl;
            std::cout << "  Time: " << duration.count() << " ms" << std::endl;

            auto metrics = ComputeQuality(testVertices, testTriangles);
            PrintMetrics("  Final Mesh", metrics);

            SaveOBJ("remesh_lloyd_only.obj", testVertices, testTriangles);
            std::cout << "  Saved to: remesh_lloyd_only.obj" << std::endl;
        }
        else
        {
    ...
    // Test 2: Lloyd + Newton
    std::cout << "\n===== Test 2: Lloyd + Newton (3 + 5 iterations) =====" << std::endl;
    {
        auto testVertices = vertices;
        auto testTriangles = triangles;
        ...
        if (MeshRemesh<double>::Remesh(testVertices, testTriangles, params))
        {
            ...
            std::cout << "SUCCESS: Lloyd + Newton remeshing complete" << std::endl;
            std::cout << "  Time: " << duration.count() << " ms" << std::endl;

            auto metrics = ComputeQuality(testVertices, testTriangles);
            PrintMetrics("  Final Mesh", metrics);

            SaveOBJ("remesh_lloyd_newton.obj", testVertices, testTriangles);
            std::cout << "  Saved to: remesh_lloyd_newton.obj" << std::endl;
        }
        else
        {
            std::cout << "FAILED" << std::endl;
    ...
    // Test 3: Newton only (from initial positions)
    std::cout << "\n===== Test 3: Newton Only (5 iterations) =====" << std::endl;
    {
        auto testVertices = vertices;
        auto testTriangles = triangles;
        ...
        if (MeshRemesh<double>::Remesh(testVertices, testTriangles, params))
        {
            ...
            std::cout << "SUCCESS: Newton-only remeshing complete" << std::endl;
            std::cout << "  Time: " << duration.count() << " ms" << std::endl;

            auto metrics = ComputeQuality(testVertices, testTriangles);
            PrintMetrics("  Final Mesh", metrics);

            SaveOBJ("remesh_newton_only.obj", testVertices, testTriangles);
            std::cout << "  Saved to: remesh_newton_only.obj" << std::endl;
        }
        else
        {
            std::cout << "FAILED" << std::endl;
    ...
    std::cout << "Newton optimizer enables faster CVT convergence!" << std::endl;
    std::cout << "Compare:" << std::endl;
    std::cout << "  - remesh_lloyd_only.obj (5 Lloyd iterations)" << std::endl;
    std::cout << "  - remesh_lloyd_newton.obj (3 Lloyd + 5 Newton)" << std::endl;
    std::cout << "  - remesh_newton_only.obj (5 Newton from scratch)" << std::endl;
