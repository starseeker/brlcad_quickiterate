...
set(CBDIR "/home/runner/work/brlcad_quickiterate/brlcad_quickiterate/brlcad_build_notls/regress/gcv/fastgen")
set(CSDIR "/home/runner/work/brlcad_quickiterate/brlcad_quickiterate/brlcad/regress/gcv/fastgen")
...
foreach(of ${OUTPUT_FILES})
  execute_process(
    COMMAND "/usr/local/bin/cmake" -E remove -f "${of}"
  )
...
execute_process(
  COMMAND "/usr/local/bin/cmake" -E tar xvf "${CSDIR}/fastgen_unix.tbz2"
...
execute_process(
  COMMAND "/usr/local/bin/cmake" -E tar xvf "${CSDIR}/fastgen_dos.tbz2"
  OUTPUT_VARIABLE fastgen_log
  ERROR_VARIABLE fastgen_log
  WORKING_DIRECTORY "${CBDIR}"
)
file(APPEND "${LOGFILE}" "${fastgen_log}")
execute_process(
  COMMAND "/usr/local/bin/cmake" -E compare_files "${CBDIR}/fastgen_dos.fast4" "${CBDIR}/fastgen_unix.fast4"
  RESULT_VARIABLE fastgen_result
  OUTPUT_VARIABLE fastgen_log
  ERROR_VARIABLE fastgen_log
  WORKING_DIRECTORY "${CBDIR}"
...
execute_process(
  COMMAND "/usr/local/bin/cmake" -E copy "${CBDIR}/fastgen_unix.fast4" "${CBDIR}/fastgen.fast4"
  RESULT_VARIABLE fastgen_result
  OUTPUT_VARIABLE fastgen_log
  ERROR_VARIABLE fastgen_log
  WORKING_DIRECTORY "${CBDIR}"
...
if(EXISTS "${CBDIR}/fastgen_unix.g")
  execute_process(COMMAND "/usr/local/bin/cmake" -E remove "${CBDIR}/fastgen.fast4")
...
execute_process(
  COMMAND "/usr/local/bin/cmake" -E remove "${CBDIR}/fastgen.fast4"
  OUTPUT_VARIABLE fastgen_log
  ERROR_VARIABLE fastgen_log
  WORKING_DIRECTORY "${CBDIR}"
)
file(APPEND "${LOGFILE}" "${fastgen_log}")
execute_process(
  COMMAND "/usr/local/bin/cmake" -E copy "${CBDIR}/fastgen_dos.fast4" "${CBDIR}/fastgen.fast4"
  OUTPUT_VARIABLE fastgen_log
  ERROR_VARIABLE fastgen_log
  WORKING_DIRECTORY "${CBDIR}"
)
