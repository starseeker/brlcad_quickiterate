...
set(LOGFILE "${CBDIR}/regress-g-assbin.log")
...
set(ofiles  "/home/runner/work/brlcad_quickiterate/brlcad_quickiterate/brlcad_build_notls/regress/gcv/assetimport-binary/conv-tetra.assbin")
foreach(of ${ofiles})
  file(REMOVE "${of}")
...
set(log_msg "Running:
  COMMAND \"${GCV_EXEC}\" \"--output-format=assetimport\" \"${in_path}\" \"/home/runner/work/brlcad_quickiterate/brlcad_quickiterate/brlcad_build_notls/regress/gcv/assetimport-binary/conv-tetra.assbin\"\n\n")
...
foreach(out_file ${ofiles})
  if(NOT EXISTS "${out_file}")
    ...
    if("${file_size}" GREATER "1490")
      math(EXPR size_delta "${file_size} - 1490")
    else("${file_size}" GREATER "1490")
      math(EXPR size_delta "1490 - ${file_size}")
    endif("${file_size}" GREATER "1490")

    if("${size_delta}" GREATER "350")
      file(APPEND "${LOGFILE}" "FAILURE: ${curr_result} file size is ${file_size} for ${out_file}\n    Expecting 1490 with delta 350")
